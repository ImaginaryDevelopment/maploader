...
let serializer =
    { new Serializer with
        ...
        override _.Serialize<'t>(x: 't) = JsonConvert.SerializeObject(x)

        [<RequiresExplicitTypeArguments>]
        override _.Deserialize<'t>(x: string) = JsonConvert.DeserializeObject<'t>(x) }
...
let fsSerializer =
    { new FsSerializer with
        [<RequiresExplicitTypeArguments>]
        override _.TryDeserializeFile<'t> path =
        ...
        [<RequiresExplicitTypeArguments>]
        override _.SerializeToFile (x: 't) path =
            fsWrapper.WriteAllText(path, serializer.Serialize<'t> x) }


let get_tf2_folder () =
    getTF2Dir (fsWrapper, (fun () -> Console.ReadLine()))

let config =
    get_tf2_folder () |> getConfig fsSerializer
...
let getMaps () = getMaps fsWrapper config
...
let rec start () =
    ...
    let rec displayMaps top page (maps: string List) =
        ...
        let redraw i =
            if Console.CursorTop = (top + ((1 + page) * config.pageSize)) then
                ...
                colorprintf (
                    ColorPrintFormat(
                        sprintf "  %s %s" (maps.[Console.CursorTop - top]) (getUsername maps.[Console.CursorTop - top])
                    )
                )
            ...
            if Console.CursorTop = (top + ((1 + page) * config.pageSize)) then
                ...
                colorprintf (
                    ColorPrintFormat(
                        sprintf
                            "$green[> %s] %s"
                            (maps.[Console.CursorTop - top])
                            (getUsername maps.[Console.CursorTop - top])
                    )
                )
        ...
        let _m =
            maps |> List.skip ((page * config.pageSize))

        _m
        |> List.take (
            if _m.Length > config.pageSize then
                config.pageSize
            else
                _m.Length
        )
        ...
        if _m.Length >= config.pageSize - 1 then
            let _m2 =
                maps |> List.skip (((1 + page) * config.pageSize))

            colorprintfn
                "  $darkgray[[Load %i more...\]]"
                (if _m2.Length >= config.pageSize then
                     config.pageSize
                 else
                     _m2.Length)
        ...
        while true do
            ...
            match key.Key with
            | ConsoleKey.UpArrow when Console.CursorTop > top -> redraw -1
            | ConsoleKey.DownArrow when
                Console.CursorTop < (top + ((1 + page) * config.pageSize)
                                     - (if _m.Length < config.pageSize then
                                            config.pageSize - _m.Length + 1
                                        else
                                            0))
                ->
                redraw 1
            | ConsoleKey.Enter ->
                ...
                proc.StartInfo.Arguments <-
                    sprintf "-hijack +map %s %s" (maps.[Console.CursorTop - top]) config.launchOptions

                proc.Start() |> ignore
            | ConsoleKey.F when
                cache.ContainsKey(maps.[Console.CursorTop - top])
                && cache.[maps.[Console.CursorTop - top]].forumid
                   <> 0
                ->
        ...
        if cmd.StartsWith "!" then
            ...
            |> Array.iter (fun x ->
                ...
                | "actf" ->
                    ...
                    firstDesc <-
                        [ "A/D Capture the Flag" ]
                        |> List.append firstDesc
                | "aprilfools"
                | "af" ->
            ...
            if last.Length > 0 then
                output <-
                    output
                    + sprintf "_(%s)[^_]*" (last |> String.concat "|")

            let message =
                sprintf
                    "%s%s%s"
                    (firstDesc
                     |> List.map (fun x -> sprintf "$green[%s]" x)
                     |> String.concat "$gray[, ]")
                    (if firstDesc.Length > 0 && lastDesc.Length > 0 then
                         "$gray[ where ]"
                     else
                         "")
                    (lastDesc
    ...
    if command.StartsWith "@" then
        ...
        let filteredMaps =
            maps |> List.filter (fun x -> filter.IsMatch x)
...
let initialiseCache () =
    ...
    let _m =
        maps
        |> List.filter (fun x -> cache.ContainsKey x |> not)
    ...
    |> List.iteri (fun i map ->
        ...
        try
            let res =
                wc.DownloadString(sprintf "https://api.skylarkx.uk/mapinfo?map=%s" map)

            let api =
                res |> JsonConvert.DeserializeObject<apimapinfo>

            cache <-
                cache.Add(
                    map,
                    { userid = api.forum_user_id
                      forumid = api.forum_id }
                )
            //printfn "'%s':%A %b" (api.map) (api.forum_user) (api.map = null)

            if api.map <> null then
                if users.ContainsKey(api.forum_user_id) then
                    users <- users.Remove(api.forum_user_id)

                users <-
                    users.Add(
                        api.forum_user_id,
                        { username = api.forum_username
                          userid = api.forum_user_id
                          display = api.forum_user.display
                          updated = api.forum_user.updated }
                    )

            if api.map <> null
               && alreadyParsedUsers.ContainsKey api.forum_user_id
                  |> not then
                alreadyParsedUsers <-
                    alreadyParsedUsers.Add(
                        api.forum_user_id,
                        { username = api.forum_user.username
                          userid = api.forum_user.id
                          display = api.forum_user.display
                          updated = api.forum_user.updated }
                    )
        ...
        with
        | e -> cache <- cache.Add(map, { userid = 0; forumid = 0 }))
    ...
    let needed =
        cache
        |> Map.toList
        |> List.map (fun (_, v) -> v.userid)
        |> List.distinct
        |> List.filter (fun x ->
            users.ContainsKey x
            && users.[x].updated < unixtime)
    ...
    let _m2 =
        wc.DownloadString(
            sprintf
                "https://api.skylarkx.uk/userinfo?user=%s"
                (needed
...
let () =
    ...
    colorprintfn
        "$yellow[Map Loader %s by Skylark#6969.\nPlease see https://skylarkx.uk/maploader for help.\nThere are %i maps available]"
        version
        maps.Length
    ...
    let latestVersion =
        (new WebClient())
            .DownloadString(
                sprintf "https://raw.githubusercontent.com/teenangst/maploader/main/version.txt?%i" unixtime
            )
            .Split('\n')

    if latestVersion.[0] <> version then
        colorprintfn
            "$white;darkred[New version %s available! Download at https://skylarkx.uk/maploaderrelease]"
            latestVersion.[0]

        latestVersion
        |> Array.take (Array.findIndex (fun line -> line = version) latestVersion)
