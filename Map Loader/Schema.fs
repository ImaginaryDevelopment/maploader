...
type apimapinfo =
    { map: string
      ...
      forum_user: apiforumuser
      discord_user_handle: string
      added: int64 }
...
type IFileInfo =
    abstract member CreationTime : System.DateTime
...
type FsSerializer =
    [<RequiresExplicitTypeArguments>]
    abstract TryDeserializeFile<'t> : string -> 't option

    [<RequiresExplicitTypeArguments>]
    abstract SerializeToFile<'t> : 't -> string -> unit
