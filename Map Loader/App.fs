...
let getTF2Dir (fs: FsWrapper, fReadLine) =
    ...
    if fs.DirectoryExists def then
        def
    else
        let rec getting () =
            ...
            if fs.DirectoryExists dir
               ...
               && Path.Combine(dir, @"hl2.exe") |> fs.FileExists then
                dir
            else
                getting ()
...
let redraw
    (cc: CConsole.ColorConsole)
    (getUsername, displayMaps)
    { Maps = maps
...
let rec displayMaps config cc (pw: ProcWrapper) (getUsername, start) cache top page (maps: string List) =
    let redraw i =
        redraw
            cc
            (getUsername, displayMaps config cc pw (getUsername, start) cache)
            { Maps = maps
              Config = config
              Top = top
              Page = page
    ...
    while true do
        ...
        match key.Key with
        | ConsoleKey.UpArrow when cc.CursorTop > top -> redraw -1
        | ConsoleKey.DownArrow when
            cc.CursorTop < (top + ((1 + page) * config.pageSize)
                            - (if _m.Length < config.pageSize then
        ...
        | ConsoleKey.Enter ->
            ...
            cc.ColorPrintf(
                ColorPrintFormat(
                    sprintf "$magenta[> %s] %s" (maps.[cc.CursorTop - top]) (getUsername maps.[cc.CursorTop - top])
                )
            )
        ...
        | ConsoleKey.F when
            cache
            |> Map.containsKey (maps.[cc.CursorTop - top])
            && cache.[maps.[cc.CursorTop - top]].forumid <> 0
            ->
            sprintf @"https://tf2maps.net/downloads/%i/" cache.[maps.[cc.CursorTop - top]].forumid
            |> launchBrowser
        | ConsoleKey.A when
            cache.ContainsKey(maps.[cc.CursorTop - top])
            && cache.[maps.[cc.CursorTop - top]].forumid <> 0
            ->
